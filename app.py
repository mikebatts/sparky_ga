...
def fetch_data():
    ...
    if property_type == "GA4":
        ...
        print("Raw JSON:", combined_response_data)

        # New code: Convert the response to JSON format
        # Process JSON data (if any preprocessing is required)
        processed_json_data = process_json_data(combined_response_data)

        # Before calling initialize_langchain_agent
        print("Processed JSON Data:", processed_json_data)


        # Initialize LangChain agent with the processed JSON data
        json_agent = initialize_langchain_agent(processed_json_data)

        # Define a human message based on the user's query
        human_message = "Analyze the JSON data to identify the top three sources of user engagement based on session data. Highlight any significant differences in engagement across different device categories."
        
        # Create the human message prompt
        human_message_prompt = HumanMessagePromptTemplate.from_template("{text}")
        chat_prompt = ChatPromptTemplate.from_messages([human_message_prompt])
        
        # Format and run the chat model
        answer = json_agent.run(
            chat_prompt.format_prompt(text=human_message).to_messages()
        )


        # Return the JSON data along with any insights generated by LangChain
        print("LangChain Insight:", answer)
        return jsonify({
            'data': combined_response_data,
            'langchain_insight': answer
        })
